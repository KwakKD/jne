    ...
    return (
        <div className="min-h-screen bg-slate-50/50">
            ...
            <main className="container mx-auto px-6 py-12">
                <Tabs defaultValue={groups[0]} className="grid grid-cols-1 md:grid-cols-[220px_280px_1fr] gap-6 items-start">
                        ...
                        <TabsList className="flex flex-col h-auto w-full bg-transparent p-0 gap-1  items-stretch justify-start">
                            {groups.map(group => (
                                <TabsTrigger
                                    ...
                                    onClick={() => setSelectedSubject(null)} // 군을 바꾸면 선택 과목 초기화
                                >
                                    {group}
                                    ...
                                    {NAVI_SUBJECT_DATA
                                        ...
                                        .map((subject, idx) => (
                                            <button
                                                ...
                                                className={`w-full text-left px-4 py-3 rounded-xl transition-all flex flex-col gap-1 mb-1
                                ${selectedSubject?.subjectName === subject.subjectName
                                                        ? "bg-blue-50 text-blue-700 ring-1 ring-blue-100"
                                                        : "hover:bg-slate-50 text-slate-600"}`}
                                            >
                                                <span className="font-bold text-[16px]">{subject.subjectName}</span>
                                                <div className="flex gap-1 items-center">
                                                    <span className="text-[10px] opacity-90">{subject.subjectType}</span>
                                                    {subject.suneung !== '-' && (
                                                        <span className="text-[10px] text-rose-500 font-bold ml-1">● 수능</span>
                                                    )}
                                                    {subject.achievement !=='' && (
                                                        <span className="text-[10px] text-emerald-600 font-bold ml-1">● 성취도 {subject.achievement}</span>
                                                    )}
                                                    {subject.rank !== '-' && (
                                                        <span className="text-[10px] text-blue-600 font-bold ml-1">● {subject.rank}</span>
                                                    )}
                                                </div>
                                            </button>
                                        ))
